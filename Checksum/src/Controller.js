...
function ToMD5Hash( Value )
{
	if( Value === undefined )
	{
		console.log( "Inside ToMD5Hash with undefined value" );
	}
	else
	{
	...
	var InitialMsg = Array.prototype.map.call( ValueStr, function( x ){
	...
	var TmpOutput = DigestToString( Digest );
	console.log( "Leaving ToMD5Hash with hash '" + TmpOutput + "'" );
...
function SetStyle( )
{
	RGBStr = RGBToColor( this.Red, this.Green, this.Blue )
	TempStr = ToFontSize( this.Size );
	
	this.MyStyle = {'font-size':TempStr, 'color':RGBStr}
	console.log( "New style is : {font-size:" + this.MyStyle['font-size'] + ", color:" + this.MyStyle['color'] + "}" );
}
...
{
	this.Hash = ToMD5Hash( this.InputString );
}

app.controller( "ChecksumController", function( $scope )
{
	$scope.Size = 20;
	$scope.Red = 0;
	$scope.Green = 0;
	$scope.Blue = 0;
	$scope.MyStyle = {'font-size':ToFontSize( $scope.Size )}
	$scope.Increase = angular.bind( $scope, IncreaseSize );
	$scope.Decrease = angular.bind( $scope, DecreaseSize );
	$scope.ChangeColor = angular.bind( $scope, SetStyle );
	$scope.Hash = "";
	$scope.InputString = ""
	$scope.InputChanged = angular.bind( $scope, InputChanged );
});